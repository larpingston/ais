#!/bin/bash

check_connection() {

        clear 
            ping gnu.org -c 2 -q -w 2 &>/dev/null 


    if [ $? = 0 ]; then

       clear

            echo -e "${GREEN}You are connected to the internet ${RESET}" 

        clear

    else

        clear

            echo -e "${RED}You are NOT connected to the internet ${RESET}\n" 

            echo -e "If you are using wifi do: \n
                    connmanctl \n
                    technologies \n
                    enable wifi \n
                    agent on \n
                    services \n
                    connect service (just do tab as service to your network) \n
                    type password \n
                    exit\n "
            exit 1
    fi
}



check_uefi() {


local uefi=""
local virt_type="none"
local vm_detected="false"
local dmi_vendor=""
local dmi_product=""
local dmi_bios=""

if [ -r /sys/firmware/efi/fw_platform_size ]; then
    uefi="$(cat /sys/firmware/efi/fw_platform_size 2>/dev/null)"
fi

if command -v systemd-detect-virt >/dev/null 2>&1; then
    virt_type="$(systemd-detect-virt 2>/dev/null)"
fi

if [[ $virt_type != "none" && -n $virt_type ]]; then
    vm_detected="true"
fi

# Fallback VM detection for non-systemd systems (e.g. Artix/OpenRC).
if [ -r /sys/class/dmi/id/sys_vendor ]; then
    dmi_vendor="$(tr '[:upper:]' '[:lower:]' < /sys/class/dmi/id/sys_vendor 2>/dev/null)"
fi
if [ -r /sys/class/dmi/id/product_name ]; then
    dmi_product="$(tr '[:upper:]' '[:lower:]' < /sys/class/dmi/id/product_name 2>/dev/null)"
fi
if [ -r /sys/class/dmi/id/bios_vendor ]; then
    dmi_bios="$(tr '[:upper:]' '[:lower:]' < /sys/class/dmi/id/bios_vendor 2>/dev/null)"
fi

if [[ "$dmi_vendor $dmi_product $dmi_bios" =~ (qemu|kvm|vmware|virtualbox|xen|bochs|bhyve|parallels|hyper-v|innotek) ]]; then
    vm_detected="true"
fi

if [ -r /proc/cpuinfo ] && rg -q 'hypervisor' /proc/cpuinfo; then
    vm_detected="true"
fi

    if [[ $uefi = "64" || $uefi = "32" ]]; then

        clear

            echo -e "${GREEN}You are using UEFI ${RESET}" 

        clear

    elif [[ $vm_detected = "true" ]]; then

        clear

        if [[ $virt_type = "none" || -z $virt_type ]]; then
            virt_type="dmi/cpuinfo"
        fi
        echo -e "${YELLOW}No UEFI firmware detected, but virtualization was found (${virt_type}).${RESET}"
        echo -e "${YELLOW}Continuing in VM compatibility mode.${RESET}"
        sleep 1

    else

        clear

        echo -e "${RED}You are NOT using UEFI ${RESET}"
        echo -e "${RED}Use an UEFI booted environment or run this inside a VM.${RESET}"

        exit 1

    fi
}



check_root() {


    if [ $EUID != 0 ]; then

        clear

        echo -e "${RED}You have to run script as root user, do sudo ./ ${RESET}"
        
        exit 1
    
    else 

        clear

            echo -e "${GREEN}You are running script as root ${RESET}" 

        clear

    fi
}
